...
public class PathSum_ii_Solution_Test {
    ...
    private void validate(List<List<Integer>> expected, List<List<Integer>> result) {
        ...
        IntStream.range(0, expected.size())
                .filter(count -> {
                    Assertions.assertEquals(expected.get(count).size(), result.get(count).size());
                    return true;
                })
                .filter(count -> {
                    IntStream.range(0, expected.get(count).size())
                            .filter(innerCount -> {
                                Assertions.assertEquals(expected.get(count).get(innerCount), result.get(count).get(innerCount));
                                return true;
                            })
